...
class Teachability(AgentCapability):
    ...
    def __init__(
        ...
        self.Action_Agent_Task = None # the task is passed from ActionAgent; parse it to know the task
    ...
    #     task = extract_tasks_as_string(text)
    #     expanded_text = text
    #     if not self.has_done_retrieval and self.memo_store.last_memo_id > 0:
    #         expanded_text = self._consider_memo_retrieval(task)
    #         self.has_done_retrieval = True
    #     pdb.set_trace()
    #     return expanded_text

   # order follows last_received_message 
    def process_all_messages_before_reply(self, text: Union[dict, str, list]): # NOTE: logic here is fragile (from upstream)
        ...
        if isinstance(text, list):
            ...
            tasks= [
                msg.get('content', '') for msg in text if isinstance(msg, dict) and msg.get('name') == 'Draft_Action_Code'
            ]
             # **assign only on first occurrence**
            if self.Action_Agent_Task is None and tasks:
                self.Action_Agent_Task = tasks[0]  # assign once, never modified afterwards
            
        elif isinstance(text, dict):
            formatted_text = f"{text.get('role', 'unknown')}: {text.get('name', 'unknown')} -> {text.get('content', '')}"
             # **assign only on first occurrence**
            if self.Action_Agent_Task is None and text.get('name') == 'Draft_Action_Code':
                self.Action_Agent_Task = text.get('content', '')  # assign once
        else:
            formatted_text = str(text)
            if self.Action_Agent_Task is None :
                self.Action_Agent_Task = formatted_text
        #### memory logic
        self._consider_memo_storage(formatted_text)
        # pdb.set_trace()
        #### retrieval logic
        if not self.has_done_retrieval and self.memo_store.last_memo_id> 0:
                self.expanded_text = self._consider_memo_retrieval( self.Action_Agent_Task)
