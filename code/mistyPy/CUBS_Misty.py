...
def load_openai_config():
    """
    从OAI_CONFIG_LIST.json文件中加载OpenAI配置

    Returns:
        dict: 包含API key和model的配置字典，如果加载失败则返回默认值
    """
    try:
        # 首先尝试从当前文件所在目录的上级目录查找配置文件
        ...
        if os.path.exists(config_path):
            with open(config_path, "r", encoding="utf-8") as f:
                ...
                if config_list and len(config_list) > 0:
                    config = config_list[0]  # 使用第一个配置
                    print(f"[CONFIG] 成功从 {config_path} 加载配置")
                    return {
                        "api_key": config.get("api_key", "YOUR_OPENAI_API_KEY_HERE"),
                        "model": config.get("model", "gpt-4o"),
                    }
        else:
            print(f"[CONFIG] 配置文件未找到: {config_path}")
    except Exception as e:
        print(f"[CONFIG] 加载配置文件时出错: {e}")
...
class Robot(RobotCommands):  # 继承与command
    def __init__(self, ip="127.0.0.1"):
        ...
        print(f"[CONFIG] 使用模型: {self.openai_model}")
        if self.oai_project_key == "YOUR_OPENAI_API_KEY_HERE":
            print(
                "[WARNING] 检测到默认API key，请在OAI_CONFIG_LIST.json中设置您的OpenAI API key"
            )
