...
class Teachability(AgentCapability):
    ...
    def process_all_messages_before_reply(self, text: Union[dict, str, list]):
      
        # ensure text is a list
        if isinstance(text, list):
            # extract and join dict values
            formatted_text = " | ".join(
        ...
        #print("ALL: This is the message where I hook " + formatted_text)
        return text
        
    def _consider_memo_storage(self, comment: Union[dict, str]): # modified from upstream
        ...
        #     comment,
        #     "Does any part of the text explicitly have the word MEM? Answer with one word: yes or no."
        # )
        ...
        if "yes" in response.lower():
            ...
            Gen_json = self._analyze(
                comment,
                "Give me the final version of the JSON that satisfies / was approved by the user. Don't make any changes to the json. If there's no json , just respond with 'jsonnotexist1926817'",
            )
            
            if "jsonnotexist1926817" not in Gen_json.lower():
                ...
                general_task = self._analyze(
                    comment,
                    "Summarize very briefly the task described in this conversation. No problem-solving details such as tell a story, take a photo,take a video, etc, NO MORE THAN FIVE WORDS",
                                    )
                # Add the task-advice (problem-solution) pair to the vector DB.
                if self.verbosity >= 1:
                    print(colored("\nREMEMBER THIS TASK-ADVICE PAIR", "light_yellow"))
                self.memo_store.add_input_output_pair(general_task, Gen_json)
                memo_added = True
        if memo_added:
    ...
    def _consider_memo_retrieval(self, comment: Union[dict, str]):
        # append the retrial only when we start a new task
        group_manager = None
        ...
        if group_manager is not None:
            if len(self.teachable_agent.chat_messages[group_manager]) > 1:
                #pdb.set_trace()
                return comment
        ...
        if "yes" in response.lower():
            ...
            general_task = self._analyze(
                task,
                "Summarize very briefly, in general terms, the type of task described in the conversation,no more than Five words",
            )
