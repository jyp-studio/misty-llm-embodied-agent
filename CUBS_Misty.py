...
sys.path.append("/Users/xiaowang/Documents/AutoMisty/AutoMistyAISymposium/code/mistyPy")
...
def event_filter(
    name: str, comparison_operator: str, comparison_value: Any
) -> Dict[str, Any]:
    """
...
def load_openai_config():
    """
    Load the OpenAI configuration from OAI_CONFIG_LIST.json.

    Returns:
        dict: config with api key and model; defaults on failure
    """
    try:
        # first look for the config file in the parent directory
        current_dir = os.path.dirname(os.path.abspath(__file__))
        config_path = os.path.join(current_dir, "..", "..", "OAI_CONFIG_LIST.json")

        if not os.path.exists(config_path):
            # fall back to the project root
            config_path = "/Users/xiaowang/Documents/AutoMisty/AutoMistyAISymposium/OAI_CONFIG_LIST.json"

        if os.path.exists(config_path):
            with open(config_path, "r", encoding="utf-8") as f:
                config_list = json.load(f)
                if config_list and len(config_list) > 0:
                    config = config_list[0]  # use the first config entry
                    print(f"[CONFIG] Loaded config from {config_path}")
                    return {
                        "api_key": config.get("api_key", "YOUR_OPENAI_API_KEY_HERE"),
    ...
    return {"api_key": "YOUR_OPENAI_API_KEY_HERE", "model": "gpt-4o"}


## inference https://docs.mistyrobotics.com/misty-ii/web-api/api-reference/#playaudio
class Robot(RobotCommands):  # inherits robot commands
    def __init__(self, ip="127.0.0.1"):
        ...
        self.oai_project_key = openai_config["api_key"]
        self.openai_model = openai_config["model"]

        print(f"[CONFIG] Using model: {self.openai_model}")
        if self.oai_project_key == "YOUR_OPENAI_API_KEY_HERE":
            print(
                "[WARNING] Default API key detected; set your key in OAI_CONFIG_LIST.json"
            )
    ...
    @staticmethod
    def is_silent(
        audio_data: np.ndarray, sample_rate: int, silence_threshold_db: int = -40
    ) -> bool:
        """
    ...
    def start_av_stream(
        self, port: int = 1935, width: int = 640, height: int = 480
    ) -> None:
        """
        ...
        res_start: Response = self.start_av_streaming(
            f"rtspd:{port}", width=width, height=height
        )
    ...
    def _read_audio_stream(self) -> None:
        ...
        try:
            container = av.open(
                self.rtsp_url, options={"rtsp_transport": "tcp", "stimeout": "5000000"}
            )
    ...
    def _process_audio(self) -> None:
        ...
        while not self._stop_event.is_set():
            try:
                ...
                current_silent: bool = self.is_silent(
                    frame_data, 44100, self.silence_threshold_db
                )
                ...
                if not current_silent:
                    ...
                    if self.voice_active and self.last_voice_time:
                        ...
                        if silence_duration >= self.silence_duration_threshold:
                            print(
                                f">> Voice activity ended (silence duration {silence_duration:.1f}s)"
                            )
    ...
    def _handle_utterance_end(self) -> None:
        ...
        if audio_duration >= self.min_utterance_length:
            ...
            print(
                f">> Discarding short utterance ({audio_duration:.1f}s < {self.min_utterance_length}s)"
            )
    ...
    def chat_with_gpt(self, user_input: str) -> str:
        ...
        with self._analysis_lock:  # Ensure thread safety with a lock
            try:
                ...
                messages = [
                    AIMessage(content=self.chat_ai_task)
    ...
    def process_multimodal_task(self) -> None:
        ...
        while not self._stop_event.is_set():
            ...
            if self.last_transcript:
                # response = self.chat_with_gpt(self.last_transcript)
                # self.gpt_speak(response)
                # self.speak(response) new change
                self.last_transcript = ""
    ...
    def start_perception_loop(self) -> None:
        ...
        threads = [
            ...
            ),  # Processes audio and transcriptions
            threading.Thread(
                target=self._video_reader_thread, daemon=True
    ...
    def register_event(
        ...
        if (
            ...
            and callback_function.__code__.co_argcount != 1
        ):
        ...
        if not event_name:
            print(
                f"No event_name provided when registering to {event_type} - using default name {event_type}"
            )
        ...
        if event_name in self.active_event_registrations:
            print(
                f"A registration already exists for event name {event_name}, ignoring request to register again"
            )
            return None
    ...
    class Event:
        ...
        def __init__(
            ...
            self.initial_flag: bool = (
                True  # Used to handle the first message differently
            )
            self.keep_alive: bool = keep_alive
            self.callback_function: Optional[Callable[[Dict[str, Any]], None]] = (
                callback_function
            )
    ...
    def transition_led(
        ...
        **Returns:**
        ...
        if transition_type not in ["Blink", "Breathe", "TransitOnce"]:
            raise ValueError(
                "transition_type must be 'Blink', 'Breathe', or 'TransitOnce'"
            )
    ...
    def emotion_Admiration(
        self, alpha: float = 1.0, layer: str = "default", isURL: bool = False
    ) -> Response:
        """
        ...
        return self.display_image(
            fileName="e_Admiration.jpg", alpha=alpha, layer=layer, isURL=isURL
        )

    def emotion_Aggressiveness(
        self, alpha: float = 1.0, layer: str = "default", isURL: bool = False
    ) -> Response:
        """
        ...
        """
        return self.display_image(
            fileName="e_Aggressiveness.jpg", alpha=alpha, layer=layer, isURL=isURL
        )

    def emotion_Amazement(
        self, alpha: float = 1.0, layer: str = "default", isURL: bool = False
    ) -> Response:
        """
        ...
        """
        return self.display_image(
            fileName="e_Amazement.jpg", alpha=alpha, layer=layer, isURL=isURL
        )
    ...
    def emotion_Anger(
        ...
        """
        return self.display_image(
            fileName="e_Anger.jpg", alpha=alpha, layer=layer, isURL=isURL
        )

    def emotion_ApprehensionConcerned(
        self, alpha: float = 1.0, layer: str = "default", isURL: bool = False
    ) -> Response:
        """
        ...
        """
        return self.display_image(
            fileName="e_ApprehensionConcerned.jpg",
            alpha=alpha,
            layer=layer,
            isURL=isURL,
        )

    def emotion_Contempt(
        self, alpha: float = 1.0, layer: str = "default", isURL: bool = False
    ) -> Response:
        """
        ...
        """
        return self.display_image(
            fileName="e_Contempt.jpg", alpha=alpha, layer=layer, isURL=isURL
        )

    def emotion_ContentLeft(
        self, alpha: float = 1.0, layer: str = "default", isURL: bool = False
    ) -> Response:
        """
        ...
        """
        return self.display_image(
            fileName="e_ContentLeft.jpg", alpha=alpha, layer=layer, isURL=isURL
        )

    def emotion_ContentRight(
        self, alpha: float = 1.0, layer: str = "default", isURL: bool = False
    ) -> Response:
        """
        ...
        """
        return self.display_image(
            fileName="e_ContentRight.jpg", alpha=alpha, layer=layer, isURL=isURL
        )

    def emotion_DefaultContent(
        self, alpha: float = 1.0, layer: str = "default", isURL: bool = False
    ) -> Response:
        """
        ...
        """
        return self.display_image(
            fileName="e_DefaultContent.jpg", alpha=alpha, layer=layer, isURL=isURL
        )
    ...
    def emotion_Disgust(
        ...
        """
        return self.display_image(
            fileName="e_Disgust.jpg", alpha=alpha, layer=layer, isURL=isURL
        )

    def emotion_Disoriented(
        self, alpha: float = 1.0, layer: str = "default", isURL: bool = False
    ) -> Response:
        """
        ...
        """
        return self.display_image(
            fileName="e_Disoriented.jpg", alpha=alpha, layer=layer, isURL=isURL
        )

    def emotion_EcstacyHilarious(
        self, alpha: float = 1.0, layer: str = "default", isURL: bool = False
    ) -> Response:
        """
        ...
        """
        return self.display_image(
            fileName="e_EcstacyHilarious.jpg", alpha=alpha, layer=layer, isURL=isURL
        )

    def emotion_EcstacyStarryEyed(
        self, alpha: float = 1.0, layer: str = "default", isURL: bool = False
    ) -> Response:
        """
        ...
        """
        return self.display_image(
            fileName="e_EcstacyStarryEyed.jpg", alpha=alpha, layer=layer, isURL=isURL
        )
    ...
    def emotion_Fear(
        ...
        """
        return self.display_image(
            fileName="e_Fear.jpg", alpha=alpha, layer=layer, isURL=isURL
        )
    ...
    def emotion_Grief(
        ...
        """
        return self.display_image(
            fileName="e_Grief.jpg", alpha=alpha, layer=layer, isURL=isURL
        )
    ...
    def emotion_Joy(
        ...
        """
        return self.display_image(
            fileName="e_Joy.jpg", alpha=alpha, layer=layer, isURL=isURL
        )
    ...
    def emotion_Joy2(
        ...
        """
        return self.display_image(
            fileName="e_Joy2.jpg", alpha=alpha, layer=layer, isURL=isURL
        )

    def emotion_JoyGoofy(
        self, alpha: float = 1.0, layer: str = "default", isURL: bool = False
    ) -> Response:
        """
        ...
        """
        return self.display_image(
            fileName="e_JoyGoofy.jpg", alpha=alpha, layer=layer, isURL=isURL
        )

    def emotion_JoyGoofy2(
        self, alpha: float = 1.0, layer: str = "default", isURL: bool = False
    ) -> Response:
        """
        ...
        """
        return self.display_image(
            fileName="e_JoyGoofy2.jpg", alpha=alpha, layer=layer, isURL=isURL
        )

    def emotion_JoyGoofy3(
        self, alpha: float = 1.0, layer: str = "default", isURL: bool = False
    ) -> Response:
        """
        ...
        """
        return self.display_image(
            fileName="e_JoyGoofy3.jpg", alpha=alpha, layer=layer, isURL=isURL
        )
    ...
    def emotion_Love(
        ...
        """
        return self.display_image(
            fileName="e_Love.jpg", alpha=alpha, layer=layer, isURL=isURL
        )
    ...
    def emotion_Rage(
        ...
        """
        return self.display_image(
            fileName="e_Rage.jpg", alpha=alpha, layer=layer, isURL=isURL
        )
    ...
    def emotion_Rage2(
        ...
        """
        return self.display_image(
            fileName="e_Rage2.jpg", alpha=alpha, layer=layer, isURL=isURL
        )
    ...
    def emotion_Rage3(
        ...
        """
        return self.display_image(
            fileName="e_Rage3.jpg", alpha=alpha, layer=layer, isURL=isURL
        )
    ...
    def emotion_Rage4(
        ...
        """
        return self.display_image(
            fileName="e_Rage4.jpg", alpha=alpha, layer=layer, isURL=isURL
        )

    def emotion_RemorseShame(
        self, alpha: float = 1.0, layer: str = "default", isURL: bool = False
    ) -> Response:
        """
        ...
        """
        return self.display_image(
            fileName="e_RemorseShame.jpg", alpha=alpha, layer=layer, isURL=isURL
        )
    ...
    def emotion_Sadness(
        ...
        """
        return self.display_image(
            fileName="e_Sadness.jpg", alpha=alpha, layer=layer, isURL=isURL
        )

    def emotion_Sleeping(
        self, alpha: float = 1.0, layer: str = "default", isURL: bool = False
    ) -> Response:
        """
        ...
        """
        return self.display_image(
            fileName="e_Sleeping.jpg", alpha=alpha, layer=layer, isURL=isURL
        )

    def emotion_SleepingZZZ(
        self, alpha: float = 1.0, layer: str = "default", isURL: bool = False
    ) -> Response:
        """
        ...
        """
        return self.display_image(
            fileName="e_SleepingZZZ.jpg", alpha=alpha, layer=layer, isURL=isURL
        )
    ...
    def emotion_Sleepy(
        ...
        """
        return self.display_image(
            fileName="e_Sleepy.jpg", alpha=alpha, layer=layer, isURL=isURL
        )
    ...
    def emotion_Sleepy2(
        ...
        """
        return self.display_image(
            fileName="e_Sleepy2.jpg", alpha=alpha, layer=layer, isURL=isURL
        )
    ...
    def emotion_Sleepy3(
        ...
        """
        return self.display_image(
            fileName="e_Sleepy3.jpg", alpha=alpha, layer=layer, isURL=isURL
        )
    ...
    def emotion_Sleepy4(
        ...
        """
        return self.display_image(
            fileName="e_Sleepy4.jpg", alpha=alpha, layer=layer, isURL=isURL
        )
    ...
    def emotion_Surprise(
        ...
        """
        return self.display_image(
            fileName="e_Surprise.jpg", alpha=alpha, layer=layer, isURL=isURL
        )
    ...
    def emotion_Terror(
        ...
        """
        return self.display_image(
            fileName="e_Terror.jpg", alpha=alpha, layer=layer, isURL=isURL
        )
    ...
    def emotion_Terror2(
        ...
        """
        return self.display_image(
            fileName="e_Terror2.jpg", alpha=alpha, layer=layer, isURL=isURL
        )

    def emotion_TerrorLeft(
        self, alpha: float = 1.0, layer: str = "default", isURL: bool = False
    ) -> Response:
        """
        ...
        """
        return self.display_image(
            fileName="e_TerrorLeft.jpg", alpha=alpha, layer=layer, isURL=isURL
        )

    def emotion_TerrorRight(
        self, alpha: float = 1.0, layer: str = "default", isURL: bool = False
    ) -> Response:
        """
        ...
        """
        return self.display_image(
            fileName="e_TerrorRight.jpg", alpha=alpha, layer=layer, isURL=isURL
        )
...
if __name__ == "__main__":
    ...
    res = misty.register_event(
        ...
        condition=[event_filter("sensorPosition", "=", "HeadRight")],
        debounce=500,
        keep_alive=True,
